...
async function handleMessage(msg, sendResponse) {
    switch (msg.type) {
        ...
        case 'SCREENSHOT_CANDIDATE': {
            if (state.status === 'recording') {
                await captureKeyScreenshot(msg.payload);
            }
            sendResponse({ ok: true });
            return;
        }
        case 'CAPTURE_TRANSCRIPT': {
            if (state.status === 'recording') {
                state.transcript.push(msg.payload);
            }
            sendResponse({ ok: true });
            return;
        }
        ...
        case 'SET_UPLOAD_AUTH': {
            ...
            const existing = (current.docStudioUploadSettings || {});
            const next = {
...
async function waitForAudioData(timeoutMs) {
    if (state.audioWavBase64)
        return;
...
async function ensureCaptureReady(tabId) {
...
async function pingCaptureScript(tabId) {
    try {
        const resp = await chrome.tabs.sendMessage(tabId, { type: 'PING_CAPTURE' });
        return !!resp?.ok;
    }
    catch {
        return false;
...
async function persistLastRecordingSnapshot(recording) {
    try {
...
function compactRecordingForSession(recording) {
    return {
        ...recording,
...
async function captureKeyScreenshot(payload) {
    ...
    try {
        ...
        try {
            focusedDataUrl = await buildFocusedScreenshotDataUrl(rawDataUrl, payload.focusRect, screenshotSettings);
        }
        catch (error) {
            console.warn('Focused screenshot crop failed, using full screenshot', error);
            focusedDataUrl = rawDataUrl;
        }
        const thumbnailDataUrl = await createThumbnailDataUrl(focusedDataUrl, screenshotSettings.thumbnailMaxSide, screenshotSettings.thumbnailQuality);
...
function computeTimestampMs() {
    if (!state.startedAt)
        return 0;
    return Math.max(0, Date.now() - new Date(state.startedAt).getTime());
}
async function buildFocusedScreenshotDataUrl(sourceDataUrl, focusRect, settings) {
    const resolved = settings ?? resolveScreenshotSettings(state.metadata);
    return renderFixedFrameWithHighlight(sourceDataUrl, resolved.outputWidth, resolved.outputHeight, resolved.outputQuality, focusRect);
}
async function createThumbnailDataUrl(sourceDataUrl, maxSide, quality) {
    return downscaleDataUrl(sourceDataUrl, maxSide, quality);
}
async function downscaleDataUrl(sourceDataUrl, maxSide, quality) {
...
async function renderFixedFrameWithHighlight(sourceDataUrl, targetWidth, targetHeight, quality, focusRect) {
    const sourceBlob = dataUrlToBlob(sourceDataUrl);
    const bitmap = await createImageBitmap(sourceBlob);
...
function resolveScreenshotSettings(metadata) {
    const width = Math.round(clamp(Number(metadata?.screenshotWidth) || DEFAULT_SCREENSHOT_OUTPUT_WIDTH, 960, 3840));
    const height = Math.round(clamp(Number(metadata?.screenshotHeight) || DEFAULT_SCREENSHOT_OUTPUT_HEIGHT, 540, 2160));
    const quality = Math.round(clamp(Number(metadata?.screenshotQuality) || DEFAULT_SCREENSHOT_QUALITY, 60, 100));
...
async function ensureContentScriptInjectedAllTabs() {
    try {
