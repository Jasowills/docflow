...
function debugLogContent(..._args: unknown[]) {
...
let micSilenceNode: GainNode | null = null;
let micWorkletModuleUrl: string | null = null;
let micProcessorNode: ScriptProcessorNode | null = null;
let micPcmChunks: Int16Array[] = [];
let micCaptureMode: 'worklet' | 'scriptprocessor' | null = null;
let micNoDataTimer: number | null = null;
let micMediaRecorder: MediaRecorder | null = null;
let micMediaChunks: Blob[] = [];
let isMicrophoneEnabledForSession = true;
let isScreenshotCaptureEnabledForSession = true;
let lastInputValues = new WeakMap<Element, string>();
let contextCacheUrl: string | null = null;
let contextCache: AppContext | null = null;
...
window.addEventListener('message', (event: MessageEvent) => {
  ...
  const data = event.data as {
    source?: string;
    type?: string;
    payload?: { apiBaseUrl?: string; bearerToken?: string; nonce?: string };
  };
  ...
  if (data.type !== 'SET_EXTENSION_UPLOAD_AUTH') return;

  const payload = data.payload || {};
  sendRuntimeMessage({ type: 'SET_UPLOAD_AUTH', payload }, (resp) => {
    const typedResp = (resp || {}) as { ok?: boolean };
    window.postMessage(
      {
        source: 'docflow-recorder-extension',
...
function onInputCapture(e: Event) {
  const el = getEventElement(e.target) as HTMLInputElement | HTMLTextAreaElement | null;
  if (!el) return;
...
function onChangeCapture(e: Event) {
  const el = getEventElement(e.target) as
    | HTMLInputElement
    | HTMLTextAreaElement
    | HTMLSelectElement
    | null;
...
// ──────────────────────────────────────────────────────────────
function emitEvent(type: RecordingEventType, data: Record<string, unknown>) {
  if (!capturing) return;
...
type ScreenshotReason = 'click' | 'form_interaction' | 'navigation' | 'state_change';

function queueScreenshotCandidate(reason: ScreenshotReason, el?: Element) {
  if (!capturing) return;
  if (!isScreenshotCaptureEnabledForSession) return;
...
function getElementFocusRect(el: Element) {
...
function extractElementContext(el: Element) {
  const textCandidate =
...
function buildElementSelector(el: Element): string {
  ...
  if (dataAutomationId) return `${tag}[data-automation-id="${dataAutomationId}"]`;
...
function getCheckedState(el: Element): boolean | undefined {
...
function getLinkHref(el: Element): string | undefined {
...
function getSanitizedElementValue(
  el: Element,
...
function sanitizeValue(value: string | undefined, maxLen: number): string | undefined {
...
function sendRuntimeMessage(message: unknown, callback?: (resp: unknown) => void) {
  if (!extensionContextValid) return;
  try {
  ...
  if (!speechRecognition) {
    ...
    speechRecognition.onresult = (event: SpeechRecognitionEventLike) => {
      for (let i = event.resultIndex; i < event.results.length; i += 1) {
        ...
        const segment: SpeechTranscriptSegment = {
          startMs,
          endMs: startMs,
          text,
...
async function startMicrophoneCapture() {
  ...
  if (
    ...
    try {
      ...
      micWorkletNode.port.onmessage = (event: MessageEvent<Float32Array>) => {
        if (!capturing) return;
...
function startMediaRecorderCapture(stream: MediaStream) {
  if (typeof MediaRecorder === 'undefined') return;
  if (micMediaRecorder) return;
  micMediaChunks = [];
  ...
  micMediaRecorder.ondataavailable = (event: BlobEvent) => {
...
async function stopMediaRecorderCapture(): Promise<Blob | null> {
  if (!micMediaRecorder) return null;
...
async function decodeRecordedBlobToWav(blob: Blob): Promise<ArrayBuffer> {
...
async function switchToScriptProcessorCapture() {
  if (!micAudioContext || !micSourceNode) return;
  if (micWorkletNode) {
    try {
      micWorkletNode.disconnect();
    } catch {
  ...
  micProcessorNode.onaudioprocess = (event: AudioProcessingEvent) => {
    if (!capturing) return;
...
function encodePcmToWav(chunks: Int16Array[], sampleRate: number): ArrayBuffer {
